...
<body>
    <div class="facture-container">
        ...
        <div class="client-section">
            <div class="client-label">Fournisseur :</div>
            <div>{{ $achat->fournisseur->nom ?? '-' }}</div>
            @if($achat->fournisseur->telephone ?? '-')
                <div>Tél: {{ $achat->fournisseur->telephone ?? '-' }}</div>
            @endif
            @if($achat->fournisseur->adresse ?? '-')
                <div>{{ $achat->fournisseur->adresse ?? '-' }}</div>
            @endif
        </div>
        ...
        <div class="payment-info">
            <p>detail Paiement :</p>
            @foreach($achat->paiements as $p)
                <p> CFA {{ number_format($p->montant, 0, ',', ' ') }} payée le <strong>{{ $p->date_paiement }}</strong></p>
            @endforeach
        </div>
